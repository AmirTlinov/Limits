...
enum CodexAppServerTransportError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidEnvelope:
            return L10n.tr("codex.transport.invalid_envelope")
        case .missingResponse:
            return L10n.tr("codex.transport.response_missing")
        case .requestTimedOut(let method):
            return L10n.tr("codex.transport.request_timeout", method)
        case .loginTimedOut:
            return L10n.tr("codex.transport.login_timeout")
        case .processExited(let detail):
            return L10n.tr("codex.transport.process_exited", detail)
...
final class CodexAppServerTransport: @unchecked Sendable {
    ...
    private var earlyLoginCompletions: [String: AppServerLoginCompletedNotification] = [:]
    ...
    func waitForLoginCompletion(loginId: String, timeout: TimeInterval = 300) async throws -> AppServerLoginCompletedNotification {
        try await withCheckedThrowingContinuation { continuation in
            ...
            queue.async {
                if let notification = self.earlyLoginCompletions.removeValue(forKey: loginId) {
                    if notification.success {
                        continuation.resume(returning: notification)
                    } else {
                        continuation.resume(throwing: CodexAppServerTransportError.loginRejected(notification.error ?? L10n.tr("codex.transport.login_incomplete")))
                    }
                    return
                }
    ...
    private func handleNotification(method: String, paramsData: Data?) {
        ...
        guard let matching else {
            if let loginId = notification.loginId {
                earlyLoginCompletions[loginId] = notification
            }
            return
        }
        ...
        if notification.success {
            ...
            let message = notification.error ?? L10n.tr("codex.transport.login_incomplete")
